...
fn main() -> Result<(), Box<dyn Error>> {
    let encoder_dir = "models/encoder";
    let model_file: PathBuf = [encoder_dir, "saved_model.pb"].iter().collect();
    ...
    // Load an input image.
    let fname = "sample.png".to_handle(&ctx)?;
    ...
    let bundle =
        SavedModelBundle::load(&SessionOptions::new(), &["serve"], &mut graph, encoder_dir)?;
